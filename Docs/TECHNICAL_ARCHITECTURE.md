...
Срез M4.3 формализует серверную lane state machine крипов: `Marching -> Engaging -> Returning -> Marching`. При первом engagement крип фиксирует ближайшую точку текущего route segment как lane anchor. Новый target не сбрасывает этот anchor. Потеря цели, выход крипа дальше 900 units, выход цели дальше 1050 units или четыре секунды непрерывного hero aggro отменяют combat и переводят крипа в Returning. В этом состоянии новое агро запрещено до достижения anchor, после чего возобновляется движение к текущему waypoint.
...
Коллизия Pawn у крипов остаётся `Ignore`: агро и attack range определяет серверная combat state machine, поэтому жёсткие капсулы не создают физический deadlock волн. M4.3 включает мягкое RVO avoidance на server-authoritative CharacterMovement с радиусом 140 units и небольшим весом. Это улучшает separation во время марша, но не является финальным Dota-подобным body blocking; жёсткое блокирование потребует отдельной сетевой и exploit-проверки.
